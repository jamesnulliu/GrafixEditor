...
#include "Grafix/Renderer/Algorithms/Cohen_Sutherland.h"
	...
	void EditorLayer::Render()
	{
		for (auto entity : m_EditorScene->GetEntities())
		{
			...
			else if (entity.HasComponent<PolygonComponent>())
			{
				...
				clip.push_back({ 0,  m_CanvasHeight });
				clip.push_back({ m_CanvasWidth , m_CanvasHeight });
				clip.push_back({ m_CanvasWidth , 0 });
				clip.push_back({ 0, 0 });
				clip.push_back({ 0 , m_CanvasHeight });
				for (int i = 0; i < clip.size() - 1; i++)
					m_Renderer.DrawLine(clip[i], clip[i + 1], m_AuxColor, LineStyle::Dashed, 20.0f);
