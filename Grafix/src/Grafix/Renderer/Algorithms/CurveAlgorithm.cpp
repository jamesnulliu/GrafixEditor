...
namespace Grafix
{
    ...
    void CurveAlgorithm::NURBS(const std::vector<glm::vec2>& controlPoints, int order, float step, const glm::vec3& color)
    {
        int n = controlPoints.size();
        std::vector<float> knots = GenerateKnots(controlPoints.size(), order);
    ...
    std::vector<float> CurveAlgorithm::GenerateKnots(int n, int k)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < segments + 1; i++)
                increments[i] = 0.5f / segments;  // Uniform
