...
namespace Grafix
{
    ...
    {
    public:
        static void Bezier(const std::vector<glm::vec2>& point, float step, const glm::vec3& color, float lineWidth);
        // n+1个控制点，k阶，次数=k-1
        static void NURBS(const std::vector<glm::vec2>& controlPoints, int order, float step, const glm::vec3& color, std::vector<float>& knots, std::vector<float>& weights, float lineWidth);
    private:
